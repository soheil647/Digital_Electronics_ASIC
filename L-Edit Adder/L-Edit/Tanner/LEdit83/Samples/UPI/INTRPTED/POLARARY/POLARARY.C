...
void PolarArrayMacro( void )
{
   ...
   LWindow tempCellWindow;
	...
	/* Create instances of temp cell in the current Cell */
		...
		LCell_MakeVisible( cCell ); /* removes all selections */
		LCell_HomeView(cCell);
}

void polar_array_macro_register(void)
{
	LMacro_Register("Create Polar Array", "PolarArrayMacro");
}
}
polar_array_macro_register();
